...
export function fmtDurationUnit(value: string | number | null | undefined, calendarBasis: boolean): string {
  ...
  if (!Number.isInteger(n)) return calendarBasis ? 'يوم تقويمي يحتسب' : 'يوم عمل يخصم';

  if (calendarBasis) {
    if (n === 1) return 'يوم تقويمي واحد يحتسب';
    if (n === 2) return 'يومان تقويميان يحتسبان';
    if (n <= 10) return 'أيام تقويمية تحتسب';
    return 'يوما تقويميا يحتسب';
  }

  if (n === 1) return 'يوم عمل واحد يخصم';
  if (n === 2) return 'يوما عمل يخصمان';
  if (n <= 10) return 'أيام عمل تخصم';
  return 'يوم عمل يخصم';
}
...
export function fmtCalendarSpan(value?: string | number | null): string {
  ...
  if (!Number.isInteger(n)) return `من ${n.toFixed(1)} يوم تقويمي`;
  if (n === 1) return 'من يوم تقويمي واحد';
  if (n === 2) return 'من يومين تقويميين';
  if (n <= 10) return `من ${n} أيام تقويمية`;
  return `من ${n} يوما تقويميا`;
}
...
export function excludedLabel(source?: ExclusionSource | null): string {
  ...
  return names.length === 0 ? '' : `بلا احتساب: ${names.join(' · ')}`;
}
...
const BLOCKER_HINTS: Record<string, string> = {
  ...
  overlap: 'للموظف إجازة مسجلة في هذه الأيام. عدل المدى، أو صحح الإجازة القائمة من سجلها.',
  half_day_on_range: 'سجل نصف اليوم الأول في سجل مستقل، والأيام الكاملة في سجل، ونصف اليوم الأخير في سجل.',
  half_day_not_allowed: 'أزل علامة نصف اليوم، أو اختر نوعا يقبل أنصاف الأيام.',
  half_day_calendar_basis: 'هذا النوع يحسب بأيام التقويم. أزل علامة نصف اليوم.',
  type_inactive: 'اختر نوعا مفعلا. النوع المعطل يبقى في السجلات القديمة كما هو.',
  after_termination: 'إن كانت الإجازة قبل إنهاء الخدمة فاضبط المدى عليها، وإن لم تكن كذلك فلا تسجلها على عقد منتهي.',
  sick_window_edge: 'اضبط النهاية على آخر يوم قبل ذكرى النافذة، ثم سجل الباقي في سجل آخر.',
  sick_window_backdated: 'سجل اليوم المرضي الأقدم قبل الأحدث حتى تحسب الشرائح بترتيب وقوعها.',
  // ── تحويلُ الطلبات الإدارية (D-LGC) ──
  legacy_pending: 'اذهب إلى «الطلبات الإدارية» واعتمد الطلب أو ارفضه، ثم ارجع لتحويله.',
  legacy_rejected: 'الطلب المرفوض لا يسجل في سجل الإجازات.',
  legacy_no_dates: 'أضف تاريخ البداية في «الطلبات الإدارية»، أو سجل الإجازة يدويا من «تسجيل غياب».',
  legacy_already_converted: 'افتح سجل الإجازة الناتج من عمود «السجل». لا يحول الطلب مرتين.',
  balance_not_initialized: 'اضغط «تهيئة الأرصدة» أولا واختر معنى الرقم (استحقاق كامل أم متبقي)، ثم حول.',
  negative_balance_unacknowledged: 'إن كان سجلك يقول إنه أخذ أكثر من استحقاقه فأكد ذلك، وإن لم يكن كذلك فراجع الرصيد الافتتاحي.',
};
...
const WARNING_HINTS: Record<string, string> = {
  pending_holiday_in_range: 'اعتمد العطلة من «التقويم الرسمي» ثم أعد الاحتساب حتى تستثنى.',
  missing_attachment: 'أضف المستند من مستندات الموظف، أو سجل الآن وأضفه لاحقا.',
  missing_event_date: 'أدخل تاريخ الواقعة حتى يطبق النظام بنسخته في ذلك التاريخ.',
  not_initialized: 'أنشئ رصيد الإجازات حتى يبدأ الاستحقاق. والتسجيل مسموح قبل التهيئة.',
  no_hire_date: 'أضف تاريخ المباشرة في ملف الموظف حتى تحسب سنة العقد على أساسه.',
  no_working_days: 'المدى كله عطل ونهاية أسبوع. راجع التواريخ إن لم يكن ذلك مقصودا.',
  accrual_drift: 'شغل مطابقة الاستحقاق أو راجع تاريخ بدء الاستحقاق. والرقم المعروض هو رقم السجل.',
  art116_threshold: 'راجع عقد الموظف قبل تجاوز عتبة المادة ١١٦.',
  art80_threshold: 'احتفظ بسجل الإنذارات خارج المنصة. الإنذارات لا تسجل هنا ولا تحتسب.',
  art117_cap_exceeded: 'ما يتجاوز سقف النافذة يسجل بلا أجر. راجع الشرائح قبل الحفظ.',
  exceeds_max_days_per_event: 'اختصر المدة إلى الحد النظامي، أو سجل الزائد نوعا آخر.',
  outside_claim_window: 'الإجازة خارج نافذة المطالبة. سجلها إن كانت موثقة، واكتب السبب.',
  min_service_not_met: 'راجع تاريخ المباشرة. الاستحقاق يبدأ بعد اكتمال مدة الخدمة.',
  probation: 'الموظف في فترة التجربة. راجع سياسة المكتب قبل الاعتماد.',
  chain_settled: 'تمت تصفية سلسلة هذا النوع. لا يخصم بعدها رصيد ولا يضاف.',
  profile_autocreated: 'تم إنشاء ملف موارد بشرية لهذا الموظف. أكمل بياناته من صفحته.',
  art151_min_postnatal: 'راجع الحد الأدنى لإجازة ما بعد الوضع قبل اختصارها.',
  floor_applied: '',
};
