...
const BASIS_HINTS: Partial<Record<TerminationBasisCode, string>> = {
  ...
  art87_force_majeure:
    '«القوة القاهرة» لا تعريف نظامي لها، وتقديرها للمحكمة العمالية حالة بحالة. والمستند إلزامي لإثباتها.',
  art87_marriage: 'مهلتها ستة أشهر من تاريخ عقد الزواج، ويلزم إدخال هذا التاريخ.',
  art87_delivery: 'مهلتها ثلاثة أشهر من تاريخ الوضع، ويلزم إدخال هذا التاريخ.',
  art80_dismissal:
    ...
    <div className="hrl-page hrs-page">
        ...
        {editable ? (
          <div className="hrs-basis__form">
            <label className="hrs-field">
              <span className="hrs-field__l">السبب</span>
              <select
            ...
            {(basis === 'art87_marriage' || basis === 'art87_delivery') && (
              <label className="hrs-field">
                <span className="hrs-field__l">
                  تاريخ عقد الزواج أو الوضع (إلزامي) — تحسب منه المهلة
                </span>
                <input
                  className="hrs-field__c"
                  type="date"
      ...
      <section className="hrs-card">
        ...
        <p className="hrs-note">
          🔴 أساس احتساب م.٨٤ هو <strong>الأجر الفعلي</strong> لا الأساسي، ولا يستبعد منه إلا ما اتفق
          عليه كتابةً بم.٨٦.
      ...
      <section className="hrs-card hrs-final">
        ...
        <p className="hrs-note">
          {statement.is_frozen ? (
            ...
            <>
              الاعتماد يقفل القسيمة والبنود، فلا يدفع يوم مرتين ولو تقاطعت مدة التصفية مع مسير شهري
              معتمَد.
            </>
          )}
        </p>
