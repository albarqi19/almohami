...
import { fmtDate } from '../components/projects/ui';
...
const ClientProjects: React.FC = () => {
  ...
  useEffect(() => {
    ClientProjectService.list().then(setItems).catch((e: Error) => setError(e.message)).finally(() => setLoading(false));
  }, []);
            ...
            {items.map((p) => (
              <Link key={p.id} to={`/my-projects/${p.id}`} className={`cx-row prj-color-${p.color}`}>
                ...
                <span className="cx-row__meta">
                  ...
                  <span className="prj-muted">{PROJECT_STATUS_LABELS[p.status]}{p.manager_name ? ` · يديره ${p.manager_name}` : ''} · بدأ {fmtDate(p.start_date)}</span>
                  <span className="prj-bar" style={{ marginTop: 6, maxWidth: 220 }}><span className="prj-bar__fill" style={{ width: `${p.progress}%`, display: 'block' }} /></span>
                </span>
