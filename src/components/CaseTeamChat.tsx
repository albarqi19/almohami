...
/**
 * محادثة فريق القضية (غرفة التجهيز) — شات بين أعضاء المكتب (لا يظهر للعميل)
 * مع إشارة @ تُنبِّه المذكور في جرس الإشعارات وتوجّهه إلى هنا (#case-team-chat).
 *
 * «رائد الذكي» (بوابة raed_assistant_enabled): عضو افتراضي مخصص لهذه القضية —
 * يشرح التصنيف المقترح، يرشد لخطوات الرفع (ناجز/الديوان/تراضي)، يستشهد من
 * الأنظمة، ويدير مهام التجهيز. نفس سلوك وشكل شات الخدمة المبسطة (كلاسات stc-*).
 */

/** رسالة محادثة فريق القضية — نفس عقد ServiceTeamMessageItem مع case_id */
export interface CaseTeamMessageItem {
  id: number;
  case_id: number;
  user_id: number | null;
...
const RAED_VIRTUAL_MEMBER = {
  ...
  name: 'رائد الذكي',
  insertName: 'رائد',
  description: 'زميلكم الذكي في تجهيز القضية — يصنّف، يرشد للرفع، ويستدل من الأنظمة',
  virtual: true,
};

const CaseTeamChat: React.FC<{
  caseId: number;
  /* طيّ عمود المحادثة إلى شريط رفيع (زر في رأس البطاقة) */
  onCollapse?: () => void;
  /* يُستدعى حين ينفّذ رائد فعلاً (مهمة تجهيز) كي تتحدث بقية الأقسام */
  onCaseMutated?: () => void;
}> = ({ caseId, onCollapse, onCaseMutated }) => {
  const { user } = useAuth();
  const myId = user?.id != null ? Number(user.id) : null;

  const [messages, setMessages] = useState<CaseTeamMessageItem[]>([]);
  ...
  const digestRaedActions = useCallback((list: CaseTeamMessageItem[]) => {
    let mutated = false;
    for (const m of list) {
    ...
    if (mutated) onCaseMutated?.();
  }, [onCaseMutated]);

  const fetchMessages = useCallback(async (initial = false) => {
    try {
      const res: any = await CaseService.getTeamChat(caseId);
      if (res.success) {
        const list: CaseTeamMessageItem[] = res.data ?? [];
        setMessages(list);
        setRaed(res.raed ?? null);
        digestRaedActions(list);
  ...
  const send = async () => {
    ...
    const finalMentions = raed?.enabled && /@رائد(?=\s|$|[.!؟،])/u.test(body) && !mentions.includes('raed')
      ...
      : mentions;
    try {
      const res: any = await CaseService.sendTeamChatMessage(caseId, body, finalMentions);
      if (res.success) {
        const appended: CaseTeamMessageItem[] = [res.data];
  ...
  const remove = async (messageId: number) => {
    try {
      await CaseService.deleteTeamChatMessage(caseId, messageId);
  ...
  /* تنفيذ اقتراح رائد بنقرة — بصلاحية الناقر (cases.edit على المسار) */
  const runSuggested = async (messageId: number, index: number) => {
    const key = `${messageId}:${index}`;
    if (busyAction) return;
    setBusyAction(key);
    try {
      const res: any = await CaseService.executeRaedAction(caseId, messageId, index);
      if (res.success) {
  ...
  const renderRaedMessage = (m: CaseTeamMessageItem) => {
    const meta = m.meta ?? {};
  ...
  return (
    <section className="ssp2-card" id="case-team-chat">
      ...
      <div className="stc-list" ref={listRef}>
        {loading ? (
          ...
          <p className="ssp2-empty">
            لا رسائل بعد — ناقشوا تجهيز القضية هنا، واستخدم @ لتنبيه زميل{raed?.enabled ? ' أو لاستدعاء رائد الذكي ✨' : ''}.
          </p>
        ) : (
          messages.map((m) => {
      ...
      <div className="stc-compose">
        <MentionInput
          ...
          placeholder={raed?.enabled
            ? 'ناقش التجهيز... @ لتنبيه زميل — و@رائد يستدعي زميلكم الذكي (Enter للإرسال)'
            : 'ناقش التجهيز مع فريقك... @ للإشارة لزميل (Enter للإرسال)'}
          disabled={busy}
